class Product {
    constructor({ id, type, name, price, weight, imgUrl, cousine, protein, count }, basketButton) {
        ...
        this.basketPlace = `.order-panel__item-list`;
        this.catalogPlace = `.product-list`;

        // catalog props
        this.toOrderLabel = `Заказать`;
        this.orderedLabel = `Заказано`;
        this.orderedClass = `products__order-by products__order-by--active`;
        this.toOrderClass = `products__order-by`;
    ...
    setEventsInBasket() {
        this.buttonToRemove = this.basketItem.querySelector('.order-panel__item-remove-btn');
        this.buttonToIncrease = this.basketItem.querySelector('.product-count-block__plus-button');
        this.buttonToDecrease = this.basketItem.querySelector('.product-count-block__minus-button');
        ...
        this.buttonToIncrease.addEventListener('click', () => {
            ...
            this.count +=1;
            this.basketItem.querySelector('.product-count-block__value').innerText = this.count;
            ...
            this.basketItem.querySelector('.order-panel__item-product-total-price').innerText = this.count*this.price;
    ...
    renderToBasket() {
        ...
        this.basketItem.classList.add('order-panel__item');
        this.basketItem.setAttribute('data-id-in-basket', this.id)
        this.basketItem.innerHTML = `<div class="order-panel__item-top">
                                        <div class="order-panel__item-product">
                                            ...
                                            <div class="order-panel__item-product-info">
                                                ...
                                                <p class="order-panel__item-product-info-price">${this.price}</p>
                                            </div>
                                            <div class="product-count-block">
                                                <button class="product-count-block__minus-button">-</button>
                                                <p class="order-panel__item-product-count product-count-block__value">${this.count}</p>
                                                <button class="product-count-block__plus-button">+</button></div>
                                                <p class="order-panel__item-product-total-price">${this.price}</p>
                                            </div>
                                        </div>
                                    </div>
    ...
    renderToCatalog() {
        ...
        this.catalogItem.classList.add('products__item')
        ...
        this.catalogItem.innerHTML = `<img class="products__img" src="${this.imgUrl}">
