...
class Basket {
  ...
  removeProductFromBasket(id) {
    ...
    this.render();
    products.removeProductFromOrdered(id)
    basketButton.render(localStorageUtil.getProducts().length)
  ...
  render() {
      ...
      const productsList = localStorageUtil.getProducts();
      let basketProductListHtml = ``;
      let totalCost = 0;

      CATALOG.forEach(({id, name, price, imgUrl, count}) => {

        if(productsList.indexOf(id) !== -1) {
          
            basketProductListHtml += `<li class="order-panel__item">
                                    <div class="order-panel__item-top">
                                      <div class="order-panel__item-product">
                                        ...
                                        <div class="product-count-block">
                                          <button class="product-count-block__minus-button" onclick="basket.decreaseCount(${id})">-</button>
                                          <p class="order-panel__item-product-count product-count-block__value">${count}</p>
                                          <button class="product-count-block__plus-button" onclick="basket.increaseCount(${id})">+</button>
                                        </div>
                                    ...
                                    <div class="order-panel__item-bottom">
                                      ...
                                      <button class='order-panel__item-remove-btn' onclick="basket.removeProductFromBasket(${id})">
...
class Products {
  ...
  handleSetLocationStorage(element, id) {
    ...
    basketButton.render(products.length);
    basket.render();
  ...
  render() {
    ...
    CATALOG.forEach(({id, type, price, name, weight, imgUrl}) => {
      ...
      htmlCatalog += `<li class="products__item" id="${id}">
                        ...
                        <button class="products__order-by${activeClass}" onclick="products.handleSetLocationStorage(this, ${id});">
                          ${activeText}
                        </button>
                      </li>`
