...
class Basket {
  ...
  constructor(paramsOrder) {
      this.orderNumber = paramsOrder.orderNumber;
      this.status = 'Формируется';
      this.area = document.querySelector('header')
      this.discount = paramsOrder.discount;
      this.totalCost = 0;
      this.content = [];
  ...
  render() {
      ...
      basket.classList.add('header__order-panel', 'order-panel')
      basket.innerHTML += `<h3 class="order-panel__title subtitle">Заказ #${this.orderNumber}</h3>
                           <ul class="order-panel__status-bar">
                             <li class="order-panel__status">${this.status}</li>
                           </ul>
                           <div class="order-panel__item-list-titles">
                             <h4 class="order-panel__item-list-title title-pos">Позиция</h4>
                             <h4 class="order-panel__item-list-title">Количество</h4>
                             <h4 class="order-panel__item-list-title">Цена</h4>
                           </div>
                           <ul class="order-panel__item-list">
                           ...
                           <div class="order-panel__order-total-info">
                             ...
                             <div class="order-total-info__values">
                               <div class="order-total-info__value">${this.discount}</div>
                               <div class="order-total-info__value">${this.totalCost}</div>
                             </div>
                           </div>
                           <button class="order__submit" type="submit">
                             Завершить покупку
                           </button>
                           <button class="order-panel__close-btn">
                             <img src="./images/icons/cancel.svg" alt="">
                           </button>`

      this.area.appendChild(basket)
  ...
  appendProduct(product) {
    ...
    let key = product.id;
    let value = [product.type, product.price, product.name, product.weight, product.imgUrl] 
    ...
    return {key: value} 
      ...
      item.innerHTML += `<div class="order-panel__item-top">
                                   <div class="order-panel__item-product">
                                     <img class="order-panel__item-product-img" src="./images/content/order-mini.png" alt="">
                                     <div class="order-panel__item-product-info">
                                       <p class="order-panel__item-product-info-name">Лапша с морепродуктами и пряностями</p>
                                       <p class="order-panel__item-product-info-price">270</p>
                                     </div>
                                     <div class="product-count-block">
                                      <button class="product-count-block__minus-button">
                                        -
                                      </button>
                                      <p class="product-count-block__value">2</p>
                                      <button class="product-count-block__plus-button">
                                        +
                                      </button>
                                     </div>
...
const basketLabel = new BasketButton();
...
class Product {
  ...
  handleSetLocationStorage(element, id) {
    ...
    basketLabel.render(products.length)

  }

  render() {
    const productList = localStorageUtil.getProducts();
    const place = document.querySelector('.hot-dishes__list')
    let htmlCatalog = ``;
    ...
    CATALOG.forEach(({id, type, price, name, weight, imgUrl}) => {
      ...
      htmlCatalog += `<li class="products__item">
                        ...
                        <button class="products__order-by${activeClass}" onclick="p.handleSetLocationStorage(this, ${id});">
                          ${activeText}
                        </button>
                      </li>`

                    })
    place.innerHTML += htmlCatalog;
